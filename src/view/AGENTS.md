...
- **`note_editor.rs`** — popup text box showing the `NoteEditor` draft + text
  ...
  position is rendered visibly as `|`. Arrow keys move the text cursor within
  the draft (`Left`/`Right` by character, `Up`/`Down` between lines with column
  ...
  inner popup width so the editor reads as one contained window. Draft text
  wraps to the popup's inner width so long unbroken input never overflows the
  window.
