...
use super::{
    calendar_layout::{visible_date_rows, visible_dates, visible_hour_cols, visible_hours},
    calendar_view::{DATE_WIDTH, HOUR_CONTENT_WIDTH},
    theme,
};

pub(crate) fn build_grid_lines(
    state: &State,
...
pub(crate) fn render_legend(frame: &mut Frame, area: ratatui::layout::Rect) {
    let legend_width = area.width.min(24);
    let legend_height = area.height.min(12);
    let legend_area =
        ratatui::layout::Rect { x: area.x, y: area.y, width: legend_width, height: legend_height };
