...
- **`Action`** (`action.rs`) — the contract from `input` to `controller`.
  ...
  (`MoveLeft/Right/Up/Down`, `MoveWordLeft/Right`, `DeleteWord`, `Confirm`,
  `InsertNewline`, `Cancel`, `CycleView`, `Digit(u8)`, `Char(char)`, `Erase`,
  `Tick`), never raw key events. `input` produces it; the
